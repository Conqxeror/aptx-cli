# APTX CLI Configuration

APTX CLI offers several ways to configure its behavior for security testing, including environment variables, command-line arguments, and settings files. This document outlines the different configuration methods and available settings for penetration testing workflows.
...
APTX CLI uses `settings.json` files for persistent configuration. There are three locations for these files:

- **User settings file:**
  - **Location:** `~/.aptx-cli/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all APTX CLI sessions for the current user.
- **Project settings file:**
  - **Location:** `.aptx-cli/settings.json` within your project's root directory.
  - **Scope:** Applies only when running APTX CLI from that specific project. Project settings override user settings.
- **System settings file:**
  - **Location:** `/etc/aptx-cli/settings.json` (Linux), `C:\ProgramData\aptx-cli\settings.json` (Windows) or `/Library/Application Support/AptCli/settings.json` (macOS). The path can be overridden using the `APT_CLI_SYSTEM_SETTINGS_PATH` environment variable.
  - **Scope:** Applies to all APTX CLI sessions on the system, for all users. System settings override user and project settings. May be useful for security teams at organizations to have controls over users' APTX CLI setups.

**Note on environment variables in settings:** String values within your `settings.json` files can reference environment variables using either `$VAR_NAME` or `${VAR_NAME}` syntax. These variables will be automatically resolved when the settings are loaded. For example, if you have an environment variable `MY_API_TOKEN`, you could use it in `settings.json` like this: `"apiKey": "$MY_API_TOKEN"`.

### The `.aptx-cli` directory in your project

In addition to a project settings file, a project's `.aptx-cli` directory can contain other project-specific files related to APTX CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (e.g., `.aptx-cli/sandbox-macos-custom.sb`, `.aptx-cli/sandbox.Dockerfile`).
- **Security configurations** (e.g., `.aptx-cli/monkMode.md`, `.aptx-cli/security-tools.json`).
- **Target profiles** (e.g., `.aptx-cli/targets.json`).

### Available settings in `settings.json`:

- **`contextFileName`** (string or array of strings):
  - **Description:** Specifies the filename for context files (e.g., `aptx.md`, `SECURITY.md`, `monkMode.md`). Can be a single filename or a list of accepted filenames.
  - **Default:** `aptx.md`
  - **Example:** `"contextFileName": "SECURITY.md"`

- **`bugCommand`** (object):
  - **Description:** Overrides the default URL for the `/bug` command.
  - **Default:** `"urlTemplate": "https://github.com/aptx-cli/aptx-cli/issues/new?template=bug_report.yml&title={title}&info={info}"`
  - **Properties:**
...
# For example:
# RUN aptx-get update && aptx-get install -y some-package
